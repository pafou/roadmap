...
# Find all roadmap<N>.json files
roadmap_files = sorted(glob.glob("data/roadmap*.json"))

if not roadmap_files:
    print("Aucun fichier roadmap<N>.json trouvé dans le dossier data/")
    exit(1)

# Create a list to hold all individual presentations
all_presentations = []

# Process each roadmap file
for roadmap_file in roadmap_files:
    print(f"Traitement du fichier: {roadmap_file}")

    # Create a new presentation from template for each roadmap
    prs = Presentation("data/Roadmap_template.pptx")

    # Extract the number from the filename (roadmap1.json -> 1)
    file_number = os.path.basename(roadmap_file).replace("roadmap", "").replace(".json", "")

    # Load the JSON data
    with open(roadmap_file, "r", encoding="utf-8") as file:
        roadmap_data = json.load(file)

    # Use the first (and only) slide from the template
    slide = prs.slides[0]

    # Clear existing example content from the template (only example bars, not month labels)
    for shape in list(slide.shapes):
        if shape.has_text_frame:
            text = shape.text_frame.text.strip()
            if text in ["Exemple de charge sûre", "Exemple de suite logique", "Exemple de charge espérée retenue", "Exemple de charge espérée", "1"]:
                shape.text_frame.text = ""
            # Keep month labels and other template elements

    # Add this presentation to our list
    all_presentations.append(prs)
...
# End of processing for this roadmap file
print(f"Fini traitement du fichier: {roadmap_file}")

# Save each presentation to individual files
for i, prs in enumerate(all_presentations):
    prs.save(f"data/Roadmap_generee{i+1}.pptx")
